...
**Where we are:** The live coordinator is running the integrated `integration/next` build at `aa11e14` with SearXNG configured locally. All 12 workers are currently online; Laptop08 remains drained from ordinary scheduling pending its Defender investigation. The post-restart 11-worker smoke wave completed successfully, and all workers reported ForgeGrid `0.8.3 / 77a07222a435`. Laptop10 remains the Windows/386 specialist.
**Next action:** Complete the capability/toolchain audit and independent review; the Qwen browser release-gate matrix now passes for offline, automatic and forced research behavior.
...
| `integration/next` | `aa11e14` | yes | Integrated release candidate; research snippet fix, updater restart-fence fix and live coordinator build |
...
| B | Opt-in web research | Local SearXNG provider, automatic current-info detection, explicit UI toggle and source display are implemented; live browser checks pass for offline casual chat, automatic weather research and forced research. |
